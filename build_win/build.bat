...
set CCFLAGS=-mz80 --opt-code-speed --std-c11 --no-std-crt0 --nostdlib --max-allocs-per-node 500000 --sdcccall 0
set RAM_ADDR=0xc000
...
sdcc %CCFLAGS% --codeseg BANK0 --code-size %BANK0_SIZE% -c ..\src\mytestrom.c
...
sdcc %CCFLAGS% --codeseg BANK6 --code-size %BANKn_SIZE% -c ..\src\data0.c
sdcc %CCFLAGS% --codeseg BANK8 --code-size %BANKn_SIZE% -c ..\src\data1.c

sdcc %CCFLAGS% -o main.ihx --data-loc %RAM_ADDR% %BANKS_ADDR% megarom.rel  test.rel test_rev.rel data0.rel data1.rel mytestrom.rel
